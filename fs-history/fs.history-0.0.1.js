		...
		{ 
			return $(this).each(function(){
				...
				$(document).ready(function()
				{
					...
					_this.height(_this.data(plugin_name).opts.min_height);
					// start loading images
					methods.load(_this, _this.data(plugin_name).first);
					// start autoplay
					methods.autoplay(_this);
				});
				// add refresh to resize event
				$(window).on('resize', function(){
					clearTimeout( _this.data(plugin_name).resize_fn );
					_this.data(plugin_name).resize_fn = setTimeout( function(){
						methods.refresh(_this)
					}, 100);
				});
			});
